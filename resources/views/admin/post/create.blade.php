                    ...
                    <div class="card shadow-sm border-0 rounded-4">
                        <div class="card-body p-4">
                            <form method="POST"
                                ...
                                enctype="multipart/form-data" x-data="postForm(@json($post ?? null))" x-init="init()">
                                @csrf
                                @if (isset($post))
                                    @method('PUT')
                                @endif
                                ...
                                <div class="progress mb-4" style="height: 18px;">
                                    <div class="progress-bar bg-primary" role="progressbar" :style="`width: ${progress}%`"
                                        x-text="Math.round(progress) + '%'" aria-valuemin="0" aria-valuemax="100">
                                    </div>
                                </div>
                                ...
                                <div x-show="step === 4" class="mb-4" x-cloak>
                                    ...
                                    <select name="visibility" class="form-select" x-model="form.visibility"
                                        ...
                                        <option value="public" :selected="form.visibility === 'public'">Public</option>
    ...
    function postForm(post) {
        return {
            ...
            nextStep() {
                ...
                if (this.step === 3 && this.form.pdfNames.length === 0 && this.form.existingPDFs.length === 0)
                    this.errors.pdfs = 'Please upload or keep at least one PDF.';
